...
if find "$main_path/output_folders/$fasta_basename_noex/$seq_id/" -name '*CASPmap.pkl' | grep -q .; then
        ...
        if [ -f "$main_path/output_folders/$fasta_basename_noex/$seq_id/yestgt" ]; then
            # run wrapper job
            job_id1=$(sbatch --partition=example --output=/dev/null --wrap="sleep 0" | awk '{print $4}')
        ...
        # if second step not complete
            ...
            # run second step
            job_id2=$(sbatch --dependency=afterok:$job_id1 --partition=example --time=0-06:00:00 --mem=120G --cpus-per-task=20 --output=$main_path/output_files/$fasta_basename_noex/$seq_id/step2_output.txt $main_path/second_step.sh $main_path/data/$fasta_file $seq_id -1 | awk '{print $4}')
