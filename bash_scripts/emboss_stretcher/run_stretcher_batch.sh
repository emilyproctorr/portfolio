...
do 
    ...
    -bsequence <(sed -n $((2 * ${SEQ2_INDEX}))p viri_ref_seqs/Chlamydomonas_reinhardtii_clu40.fasta) | grep -E "1: |2: |Similarity" | awk '/^# 1:/ {first=substr($3, 2)} /^# 2:/ {second=substr($3, 2)} /^# Similarity:/ {match($NF, /([0-9.]+)/, perc); print first, second, perc[1]}' >> /home/scratch/stretcher/${PAIR_FILE_BASENAME}_results.txt
done